...
--CAU4:
SELECT SP.MASP,SP.TENSP,SP.DVT,SP.GIA
FROM SANPHAM SP
WHERE SP.NUOCSX='TRUNGQUOC' AND (SP.GIA>=30000 AND SP.GIA<=40000);
...
WHERE HOADON.TRIGIA = (SELECT MAX(HD.TRIGIA)
						FROM HOADON HD);
...
FROM KHACHHANG KH JOIN HOADON HD
	ON KH.MAKH=HD.MAKH
WHERE HD.TRIGIA >= (SELECT MAX (HD.TRIGIA) FROM HOADON HD);
